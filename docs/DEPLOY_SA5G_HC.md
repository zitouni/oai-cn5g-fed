...
OAI 5G core network has different network functions which can be used invidiually or deployed all together in different combination on a production grade Kubernetes cluster like Openshift or a vanilla kubernetes cluster.
...
For the moment we provide helm chart of inidividual network functions, udr, udm, ausf, amf, nrf, smf, upf and nssf. To make the deployment of the network function easier we provide three different settings in which they can be deployed.
...
In this tutorial we will deploy a basic setting of OAI 5g core network and will deploy oai-gNB and oai-nr-ue in rf-simulator mode to perform some traffic testing. You can also deploy the core network in the other two settings, it all depends on your use case and testbed. The configuration parameters of helm charts are the same as `docker-compose` so if you know how to configure `docker-compose` yaml file you can configure the helm charts `value.yaml`.
...
All the OAI core network charts are present in `oai-5g-core` folder, there you can find charts of individual network functions and for the three different deployment settings described above.
...
In the `values.yaml` file we have put only the configuration parameters which we think are really necessary and they should be changed based on PLMN, DNN and sim card information. In case you want to change some other parameters we suggest you go in the helm charts of the network function and do the change there.

Helm chart of every network function looks similar and has the structure below. Only the chart of mysql database and NRF is different.
...
All the network function related configurable parameters are in the sections `config` of the `values.yaml` file. To understand the usage and description of each network function configuration parameter ,refer to their [wiki page](https://gitlab.eurecom.fr/oai/cn5g/oai-cn5g-amf/-/wikis/home).

Create a namespace where the helm-charts will be deployed, in our environment we deploy them in `oai` namespace. To create a namespace use the command below on your cluster,
...
Network functions discover each-other using NRF and instead of using the ip-address of network functions we rely on using their FQDN, **kubernetes service concept** to communicate with each other, whether we deploy them in reference point architecture or service based architecture.
...
In case of pulling images from docker-hub, it will be good to configure an image pull secret. Currently docker allows only 100 pulls from docker-hub for an anonymous account. If you provide a personal/company account the limit will increase. If you want to provide a secret, configure this section in `values.yaml`
...
There are more ways to make docker secret, you can follow this [link](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/). After creating the secret, mention it in the `values.yaml`
...
In the smf the config value `useLocalSubscriptionInfo` makes sure that the subscriber DNN, NSSAI, ip-address mapping is taken via UDM. In case you want to use the local subscription information described in the smf you can set this parameter to `no`. In minimalist deployment this parameter is always `no`.

In case you are looking for some other parameter which isn't available in the values.yaml file of the oai-5g-basic chart then check in the individual chart of the network function.
...
**NOTE**: Whenever you make any change in the network function helm-chart or mysql helm chart, you need to perform a dependency update to inform parent chart about the sub-charts update.
...
Once the configuration is finished the charts can be deployed with a user who has the relevant rights.

1. Create RBAC
2. Run pod with privileged and anyuid scc (optional, only required if you have scc configured in your cluster)
...
**Very Important** To access internet in NR-UE the DN/SGI interface of UPF/SPGWU should be able to access the internet.

gNB requires the ip-address of the AMF which you have acquired earlier, use that ip-address and configure it in `config.amfIpAddress`. We are not using multus here just for simplicity, generally there should be two interfaces of gNB(for N2 and N3).
...
To check if gnB is connected, read the logs of amf and check if the N2 setup procedure was correct or not.
...
NR-UE requires the ip-address of the gNB which you have acquired earlier, use that ip-address and configure it in `config.rfSimulator`. We are not using multus here just for simplicity, generally there should be two interfaces.
...
Now you can start reading the logs of amf, smf and other network function to understand the message flow. Once the pdu session establishment procedure is finished you will receive an ip-address. You can start performing some testing.
...
## in case the commands above don't work, try with 8.8.8.8 instead for the dns server. If that works then you probably have't configured dns properly in SMF.
```
...
### 6.1 Changes for Vanilla Kubernetes

All the OAI containers run the network function process as they are root inside the container. In the docker engine this is not a problem because by default user is root inside the container. But in case of podman in openshift, the containers run as non root users. Some containers like SPGWU, gnb and nr-ue and the tcpdump container to capture packets are privileged containers. In this case in `specs` of these pods the `privilaged` flag should be true.

If you are using a specific user instead of `kubeadmin` or cluster-admin type to deploy the helm-charts, the only important change is related to `rbac.yaml` in the `templates` folder of each helm-chart. Currently it is configured for openshift api group `security.openshift.io` but this does not exist in Vanilla Kubernetes. Instead it should be changed with:
